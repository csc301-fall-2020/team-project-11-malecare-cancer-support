...
def get_user_id_from_admin_filter(include_cancer, exclude_cancer, include_treatment, exclude_treatment,
    ...
    start_year = datetime.datetime(datetime.datetime.utcnow().year - age_max, 1, 1)
    end_year = datetime.datetime(datetime.datetime.utcnow().year - age_min, 12, 31)
    query_result = User.objects(Q(cancer__in=include_cancer) &
                                ...
                                Q(date_of_birth__lte=end_year)).only('user_id').values_list('user_id')

    return query_result

def get_email_from_admin_filter(include_cancer, exclude_cancer, include_treatment, exclude_treatment, include_medication, exclude_medication, gender, age_min, age_max):
    start_year = datetime.datetime(datetime.datetime.utcnow().year - age_max, 1, 1)
    end_year = datetime.datetime(datetime.datetime.utcnow().year - age_min, 12, 31)
    query_result = User.objects(Q(cancer__in=include_cancer) &
                                Q(cancer__nin=exclude_cancer) &
                                ...
                                Q(date_of_birth__exists=True) &
                                Q(date_of_birth__gte=start_year) &
                                Q(date_of_birth__lte=end_year)).only('user_id').values_list('email')
